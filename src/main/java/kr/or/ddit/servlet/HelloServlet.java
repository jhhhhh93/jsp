...
// JE22 기반의 웹 어플리케이션 개발시 main method를 통해 실행하지 않는다.
...
// 3. doGet, doPost 메서드를 오버라이딩 한다.
// 4. 어떤 url을 해당 서블릿에서 처리할지 매핑(web.xml, servlet 3.0 이후 부터는 servlet class 어노테이션으로 설정)
public class HelloServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		logger.debug("HelloServelet init() : {} ", config.getInitParameter("test"));
	}
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		pw.write("	</head>");
		pw.write("	<body>");
		pw.write("		<h1>Hello, world 한글한글</h1>");
		pw.write("	</body>");
		pw.write("</html>");
	}
}
